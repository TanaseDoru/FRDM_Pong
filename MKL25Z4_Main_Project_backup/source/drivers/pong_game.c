...
//    // Desenare pereti (fara bara centrala)
//    ST7735_DrawVLine(0, 0, 128, COLOR_WHITE);
//    ST7735_DrawVLine(159, 0, 128, COLOR_WHITE);

    // Afiseaza scor initial
    char score_str[8];
    snprintf(score_str, sizeof(score_str), "%d", p1.score);
    ST7735_DrawString(10, 2, score_str, COLOR_WHITE, COLOR_BLACK);

    snprintf(score_str, sizeof(score_str), "%d", p2.score);
    ST7735_DrawString(145, 2, score_str, COLOR_WHITE, COLOR_BLACK);
}
...
void Game_Update(void) {
    ...
    if (ball.x < 0) {
        ...
        // Actualizeaza scor P2 pe ecran
        char score_str[8];
        snprintf(score_str, sizeof(score_str), "%d", p2.score);
        ST7735_FillRect(145, 2, 14, 8, COLOR_BLACK); // Sterge scor vechi
        ST7735_DrawString(145, 2, score_str, COLOR_WHITE, COLOR_BLACK);
    ...
    if (ball.x > 160) {
        ...
        // Actualizeaza scor P1 pe ecran
        char score_str[8];
        snprintf(score_str, sizeof(score_str), "%d", p1.score);
        ST7735_FillRect(10, 2, 14, 8, COLOR_BLACK); // Sterge scor vechi
        ST7735_DrawString(10, 2, score_str, COLOR_WHITE, COLOR_BLACK);
